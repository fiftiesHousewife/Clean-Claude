...
public abstract class SummaryReportTask extends DefaultTask {
    ...
    private String renderMarkdown(final Map<String, Counts> byModule,
        ...
        markdown.append("# Clean Code — self-analysis summary\n\n");
        markdown.append("Auto-generated by `./scripts/dogfood.sh` (`./gradlew cleanCodeSummary`).\n");
        markdown.append("Deterministic: contains no absolute paths, timestamps, or machine-specific\n");
        markdown.append("content, so local and CI runs produce byte-identical output. CI fails if\n");
        markdown.append("this file drifts from the committed state.\n\n");
        markdown.append("Code reference: [HEURISTICS.md](../clean-code/HEURISTICS.md) ");
        markdown.append("(extracted from the plugin jar by `./gradlew cleanCodeWriteHeuristics`).\n\n");

        markdown.append("**Total: ").append(totals.total())
                .append(" findings** (").append(totals.errors()).append(" errors · ")
                .append(totals.warnings()).append(" warnings · ")
                .append(totals.info()).append(" info)\n\n");

        markdown.append("## By module\n\n");
        markdown.append("| Module | Errors | Warnings | Info | Total |\n");
        markdown.append("|---|---:|---:|---:|---:|\n");
        byModule.forEach((name, counts) -> markdown.append("| ").append(name)
                .append(" | ").append(counts.errors())
                .append(" | ").append(counts.warnings())
                .append(" | ").append(counts.info())
                .append(" | ").append(counts.total())
                .append(" |\n"));
        markdown.append("| **Total** | **").append(totals.errors())
                .append("** | **").append(totals.warnings())
                .append("** | **").append(totals.info())
                .append("** | **").append(totals.total())
                .append("** |\n\n");

        markdown.append("## By heuristic code\n\n");
        markdown.append("| Code | Count |\n");
        markdown.append("|---|---:|\n");
