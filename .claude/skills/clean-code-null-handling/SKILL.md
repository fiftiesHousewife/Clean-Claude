---
name: clean-code-null-handling
description: Apply when fixing null-handling findings from the Clean Code plugin — Ch7.2 (don't return null). Use when the user asks to replace null returns with Optional, add fail-fast null checks with Objects.requireNonNull, or replace if (x != null) control flow with null-object patterns.
---
