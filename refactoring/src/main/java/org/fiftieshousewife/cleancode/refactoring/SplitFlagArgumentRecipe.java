...
public class SplitFlagArgumentRecipe extends Recipe {
    ...
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return new JavaIsoVisitor<>() {
            ...
            public J.ClassDeclaration visitClassDeclaration(J.ClassDeclaration classDecl,
                ...
                return splitFlagMethods(c);
            }
        };
    }

    private static J.ClassDeclaration splitFlagMethods(J.ClassDeclaration c) {
        ...
        for (final Statement stmt : original) {
            rewritten.add(stmt);
            if (!(stmt instanceof J.MethodDeclaration method) || !isSplitCandidate(method)) {
                continue;
            }
            if (appendSplitHelpers(method, rewritten)) {
                changed = true;
            }
        }
    ...
    private static boolean appendSplitHelpers(J.MethodDeclaration method, List<Statement> target) {
        ...
        target.add(whenTrue);
        target.add(whenFalse);
        return true;
    }

    private static boolean isSplitCandidate(J.MethodDeclaration method) {
        if (!isPrivate(method) || !hasSingleBooleanParameter(method)) {
            return false;
        }
    ...
    private static boolean isPrivate(J.MethodDeclaration method) {
        return method.getModifiers().stream()
                .anyMatch(m -> m.getType() == J.Modifier.Type.Private);
    }

    private static boolean hasSingleBooleanParameter(J.MethodDeclaration method) {
        final List<Statement> params = method.getParameters();
        if (params.size() != 1 || !(params.getFirst() instanceof J.VariableDeclarations vd)) {
            return false;
        }
        return vd.getType() == JavaType.Primitive.Boolean;
    }

    private static String flagParameterName(J.MethodDeclaration method) {
    ...
    private static List<Statement> asStatements(Statement body) {
    ...
    private static J.MethodDeclaration emitHelper(J.MethodDeclaration original,
                                                  String flagName,
                                                  boolean whenTrue,
                                                  List<Statement> body) {
